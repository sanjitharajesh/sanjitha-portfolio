  ...
  return (
    <div className="flex flex-wrap gap-1.5 mt-3">
      {tags.map((tag) => (
        <span
          key={tag}
          className="font-mono text-[10px] font-semibold border border-[#4a5a8a] text-[#a0b0d0] rounded-full px-2 py-0.5"
        >
          {tag}
        </span>
      ))}
...
const MacDots = ({ onClose }: { onClose?: () => void }) => (
  <>
    <span
      className="w-3 h-3 rounded-full bg-[#ff5f57] flex-shrink-0 flex items-center justify-center cursor-pointer"
      onClick={onClose}
    >
...
const projects: Project[] = [
  ...
  },
  {
    ...
    date: "Jan 2026",
    teaser: "3-stage AI document pipeline using Azure CV OCR and GPT-4-Turbo achieving 92% accuracy across 50 receipt layouts at $0.01 per document.",
  ...
  },
  {
    ...
    date: "Dec 2025",
    teaser: "Production RAG system over 141 FDA drug labels with hybrid BM25-Pinecone retrieval, Langfuse observability, and 87% accuracy with under 5% hallucination rate.",
  ...
  },
  {
    ...
    date: "Oct – Dec 2025",
    teaser: "Real-time fraud detection across 24 merchant risk tiers with NL-to-SQL admin interface via SQLCoder-7B and Ollama.",
  ...
  },
  {
    ...
    date: "Oct – Dec 2025",
    teaser: "Multilingual NLP pipeline benchmarking mT5, MarianMT, and NLLB-200 on Hinglish and Spanglish, achieving 63.12 BLEU on NLLB-200.",
  ...
  },
  {
    ...
    date: "Feb – May 2025",
    teaser: "EfficientNet-B0 fine-tuned in PyTorch for pneumonia and tuberculosis classification from chest X-ray images.",
  ...
  },
  {
    ...
    date: "Jan – May 2024",
    teaser: "Twin CNN and Bi-LSTM architectures for sleep stage classification from PSG physiological signals, 82% and 84% accuracy respectively.",
  ...
  },
  {
    ...
    date: "Oct – Dec 2023",
    teaser: "Interactive R Flexdashboard analyzing 6,000+ air quality records across 50+ cities identifying a 30% PM2.5 increase in metro areas.",
  ...
  },
  {
    ...
    date: "Jun – Jul 2023",
    teaser: "Real-time hand gesture recognition system using MediaPipe and TensorFlow achieving 86% accuracy across 500+ video samples.",
...
export default function Projects() {
  ...
  return (
    <>
      <h2 id="projects" className="font-inter font-bold text-[clamp(28px,4vw,38px)] mt-16 md:mt-64 mb-8 scroll-mt-36 text-center">
      ...
      <section className="grid md:grid-cols-2 gap-7 mt-9">
        {projects.map((project, idx) => (
          ...
          >
            ...
            <p className="font-inter text-[14px] leading-relaxed flex-1">{project.teaser}</p>
      ...
      {active && (
        ...
        >
          <div
            className="bg-[#0d1f45] border border-[#2a3a6a] rounded-2xl w-full max-w-2xl max-h-[88vh] overflow-y-auto shadow-2xl text-[#dfe8ff]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="sticky top-0 bg-[#1a2a52] px-4 py-2.5 rounded-t-2xl flex items-center gap-2 group/bar relative">
              <MacDots onClose={() => setOpen(null)} />
              ...
              <button
                onClick={() => setOpen(null)}
                className="ml-auto font-mono text-[#8899cc] hover:text-[#e9f1ff] text-xl leading-none z-10 px-1 transition-colors duration-150"
                aria-label="Close"
              >
                ×
              </button>
            ...
            <div className="p-6">
              <div className="flex items-center justify-between mb-5">
                <span className="font-mono text-[13px] font-semibold text-[#8899cc]">{active.date}</span>
              ...
              {(() => {
                ...
                return (
                  <>
                    <p className="font-mono text-[10px] font-bold text-[#6677aa] uppercase tracking-widest mb-1.5">What I built</p>
                    <p className="font-inter text-[15px] leading-relaxed text-[#dfe8ff] font-semibold">{first}</p>
                    {rest && (
                      <>
                        <p className="font-mono text-[10px] font-bold text-[#6677aa] uppercase tracking-widest mt-4 mb-1.5">Results</p>
                        <p className="font-inter text-[15px] leading-relaxed text-[#c8d4f0]">{rest}</p>
                      </>
                    )}
                  </>
                );
              })()}
              <ModalTags tags={active.tags} />
            </div>
          </div>
        </div>
      )}
